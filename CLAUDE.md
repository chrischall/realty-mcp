...
|---|---|
| `@chrischall/realty-core` | Pure, dependency-free helpers: address tokenisation + match scoring, USPS street-suffix variant expansion, locality-alias remap (Lake Lure ↔ Rutherfordton, Beech / Sugar Mountain ↔ Banner Elk), free-text address parsing, `calculateMortgage` (PITI), `calculateAffordability` (DTI inverse-PMT), `sqftToAcres` (lot-size unit conversion), shared resolver types. Every export has a vitest. |
